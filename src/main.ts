...
async function bootstrap() {
  ...
  backOfficeDocument.paths = Object.keys(backOfficeDocument.paths).reduce((newPaths, path) => {
    newPaths[`/api/v${API_DEFAULT_VERSION}${path}`] = backOfficeDocument.paths[path];
    return newPaths;
  }, {});
